estimate_trend <- function(samples = nsmooth_samples,
                    scale = "strat_name",
                    start_year = 2000,
                    end_year = 2019,
                    slope = FALSE,
...
map_trends <- function(base_map = realized_strata_map,
                      trends = trend_90,
                      trend_col = "mean_trend",
                      strata_header = "strat_name",
                      species = species){
  
  trends = trends %>% 
    rename_with(.,~gsub(trend_col, "trendcol", .x, fixed = TRUE))
  ...
  tmap <- left_join(base_map,trends,by = c("ST_12" = strata_header)) %>%
    mutate(Tplot = cut(trendcol,breaks = c(-Inf, breaks, Inf),labels = labls))
